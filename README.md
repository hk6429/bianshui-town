...
同一住宅接觸多處園景時，將原始效益由高到低排序，再依1、1/2、1/4等權重相加，總上限20。全城按住戶數加權（空屋權重1）的平均加成，實際提高住宅需求及有限人口吸引目標；安置與新入住候選依「污染減園景加成」排序，各自沿用原有資格條件；新住戶仍須符合工作、供水及道路要求。自由營造只顯示參考、不套用需求加成。
...
停止生產後，污染每90遊戲秒減半。污染保存於土地，不會隨工坊搬移或拆除瞬間消失；讀檔保留殘留量，不補算離線時間。住宅以占地平均污染計算居住環境（100減污染）；城市依各住宅住戶數加權，空屋權重1，降低住宅需求最多30點；新入住與安置優先選污染較低的可用住宅。
