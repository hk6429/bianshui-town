...
「選關畫面」提供十條文學關卡的目標、前置設施與完成進度；每關共用目前小鎮，換關不清除進度。Google登入採Turso＋Cloudflare，已啟用；登入後由玩家確認上傳或預覽載入雲端城市。本機存檔與匯出照常使用。[設定與驗證](docs/cloud-save-setup.md)。

## 學習與課堂功能

十關提供「玩完我能」目標、三種可切換閱讀支援、故事示意圖、重點語詞注音、逐句朗讀、前後選讀段落與原作來源。先選答案與原文證據，再寫理由；錯答和提示保留，理由不由關鍵字自動評分。各關有原文回憶與生活遷移兩類練習，隔一天可再複習，無連續登入獎懲。作品最多保留12版，可記下回饋、修改原因並下載SVG圖文卡。

「營造與工具 → 學習紀錄與教師看板」可匯出作業。教師可產生指定關卡與閱讀支援的課堂連結；課堂使用獨立本機小鎮、備好前置設施和道路，不覆寫原城，課堂不將城市上傳到個人雲端存檔。

教師批次匯入學生JSON作業（最多40人），查看常見卡點、提示、證據與作品修訂，針對作品版本匯出人工回饋，學生匯入後可修改。**教師看板為本機檔案彙整，不是即時全班雲端同步**；學生稱呼可用座號，資料不包含Google憑證。作品理由與遷移需教師判讀，建築等級不當作學力分數。課堂15–20分鐘為設計目標，學習效果與真實學童使用仍待試玩。

[學習功能驗證](docs/learning/RELEASE.md)

## 宋韻文學營造

2026-09-20更新已部署至[正式站](https://bianshui-town.pages.dev/)與[備援站](https://bianshui-town.netlify.app/)。

入口：「營造與工具 → 宋韻任務」。十條任務結合原文閱讀、30個場景操作與四格地標：岳陽樓、開封府、醉翁亭、愛蓮池、赤壁文舟、活字印書坊、賣油翁技藝坊、溪亭荷塘、元夕燈市、東坡望月臺。

1. 完成三段閱讀及三個操作場景（物資分配、排序、配對、訪查、路徑或排字），並備齊任務列出的城鎮設施，即可解鎖。錯答不扣錢，完成後可重玩練習。
2. 選完整2×2空地營造。經營模式先鋪路，自由營造自動接路；岳陽樓須位於東緣臨河地帶，或緊鄰已落成池塘／指定四格水景。新建與搬移使用相同條件；舊存檔地標保留。
3. 從建物資訊卡排程活動：白天12時、夜間19時。居民須沿路抵達並全程停留18遊戲秒。每日每居民最多增加2點學力，不重複刷錢。
4. 升級依序完成配套設施與到場活動（二級）、操作與延伸閱讀（三級）、兩個遊戲日的公共服務（四級）、另一作品閱讀操作及到場活動、兩篇引文與20–400字比較短箋（五級）。每級增加用途專屬外觀，最高五級。

印書坊會招聘工匠，消耗紙與墨生產有來源紀錄的書籍，再由推車送到書肆銷售；須有到場工人與暢通道路，缺料或滿倉時等待。物產流轉可追查紙墨來源；耗墨不算成交，拆除後貨物回到貨棧。遊記、判案摘要、工序手記、圖文回憶與書信可在任務中記錄，切換任務保留草稿，重新整理前須按「儲存短箋」。

岳陽樓參照**現存清代形制**（黃瓦、三層飛簷、盔頂）製作四格縮尺模型，不宣稱宋樓原貌或測繪級復原；包公故事為後世戲曲與遊戲改編。其餘地標為跨地域文學意象，內置水景不冒充真實航道。[史料與模型界線](docs/landmark-history.md) · [逐項完成證據](docs/literary-completion.md)。

## 畫面設定

「營造與工具 → 畫面設定」可選依系統、減少動態或完整動態。減少動態隱藏雨線、停止裝飾擺動與鏡頭慣性，跟隨居民只定位一次；核心行走、運貨與城鎮時間仍照常。用途標籤可同時標示民居、商鋪、作坊及園景／公設，並收合額外工具以留出觀察空間。偏好只保留於目前瀏覽器，不混入城鎮存檔。證據見 [畫面設定](evidence/view-preferences/README.md)。

## 操作狀態與通知

施工小卡顯示百分比；物產列表標示目前選取批次，篩選後同步詳細內容。「營造與工具」提供手機畫卷與通知紀錄入口，最近30則操作結果可回看。時間狀態區分主動暫停與閱讀暫停，關閉視窗不解除主動暫停。通知僅保留本次開啟期間，重新整理即清空。

驗證見 [操作狀態紀錄](evidence/operation-state/README.md)。百項改善的逐項狀態與四位專家複評見 [驗收帳本](docs/review/ledger.json)及 [最終複評](docs/review/final-summary.md)。

## 城市經營、就業與人口

新玩家以 2400 文開始經營；舊存檔遷移保留自由營造，不追扣建設費。收支面板可切換模式與設定 0–20% 稅率。建屋、道路、搬移、擴建、升級共用報價與扣款規則，失敗不扣款；每個遊戲日結算落成建築和道路的維護費，入住與真實成交形成稅收。

存檔格式升為 9，新增 `city` 白名單；可讀版本 1–8。經營模式已加入住宅／商業／作坊需求，稅負會降低需求，居民每 15 遊戲秒依需求配額分批遷入，優先安置無家住戶。長期無家（90 秒）、失業（180 秒）或缺乏日用品（360 秒）才會每次最多遷出一人；改善條件會停止對應倒數。自由營造維持直接入住及不強制遷出。需求及原因可在「金庫 · 需求」查看；這些分數是遊戲規則，非史實城市經濟估計。欠款會影響公共服務效率；供水、清運、醫療、教育、防火與園景共同影響居民生活。商業周轉金與市府分帳，原料補貨須付成本；貨棧可升至三級，提高腳夫、牛車與每趟載量，並增加投資及維護費。詳見 [貿易與物流驗收](evidence/trade-logistics/README.md)。經營模式須自行把門前步道鋪到東側河岸引道；斷路會影響移入、通勤與送貨。大路提供雙排行人通行，費用與小路不同。詳見 [玩家道路驗收](evidence/road-network/README.md)。開發證據見 [財政驗收](evidence/city-finance/README.md)、[就業與採買](evidence/employment/README.md)、[城市成長](evidence/city-growth/README.md)。
...
先按「操作場景（鍵盤）」將焦點移入場景；也可用Tab移到場景。場景中按2選民居、3選商鋪、4選作坊，方向鍵或WASD移動格點，Enter放置，Esc取消；圖錄四格設計使用同一個2×2占地規則。預覽顯示座標、占地、可否放置與費用，放置沿用滑鼠的驗證、扣款及復原流程。探索模式按方向鍵或WASD平移，Q/E旋轉。

快捷鍵只在場景有焦點時生效；按鈕、連結、輸入欄保留原生鍵盤行為，Tab可離開場景。營造或搬移期間也有「取消營造」按鈕，手機可一按取消草稿而不改城市。驗證見 [鍵盤操作證據](evidence/keyboard/README.md)。

## 觸控草稿與明確確認

手機觸控及觸控筆營造時，抬起後保留完整占地草稿，不立即建造或扣款。可用四個方向鈕逐格微調，看到費用與可放置狀態後，按「確認營造」才執行；取消會回探索而不修改小鎮。桌面可勾「放開後先預覽」使用相同流程，未勾時滑鼠沿用放開提交。

第二根手指加入即取消草稿，整段雙指縮放都不能營造，全部放開後才接受下一次單指操作；指標取消、切換模式或離開分頁也會清除草稿。確認時重新檢查占地與金庫，避免使用過期的預覽結果。手機待確認時暫收起縮放鈕以免遮擋文字，完成或取消後恢復。Chrome觸控事件與手機尺寸證據見 [觸控驗收](evidence/touch-plans/README.md)。

## 可解釋的營造預覽

營造預覽用「✓ 可營造／✕ 不可營造」與文字說明，不只靠紅綠框。屋舍與道路占用、超出邊界會列出格點；也會說明不相連、重複占地、四格限定或金庫不足。

提交前顯示實際結果名稱、占地格數、建築棟數和費用：雅居小樓斜拉2×2會變成「四合雅宅・占地4格・1棟」，直拉4格則為「雅居小樓・占地4格・4棟」。名稱與占地轉型和實際建造共用規則；道路形成廣場、花園連成四格自動合併也會預告，包含需合併的既有花園格數。確認時仍重新驗證。證據見 [營造預覽驗收](evidence/plan-preview/README.md)。
...
證據見 [衛生與欠款驗收](evidence/sanitation/README.md)。健康已接上藥鋪醫療與病假；五項民生滿意已整合供水、衛生、健康與其他生活條件，詳見下節。

## 民生滿意與人口去留

每位居民都有0–100分的民生滿意：住房占20％、就業15％、採買15％、公共服務25％、環境25％。點選居民的「查看分項」可看分數、權重與原因；收支總覽顯示全城平均、低滿意人數與需求修正。

公共服務分數由供水40％、衛生20％、健康20％、醫療10％、教育5％、巡守5％組成；健康滿分代表無需醫療照護。環境分數為「80－住宅污染×0.8＋園景加成」，限制在0–100。住房依住宅容量與住戶數計算，就業須有可達且可運作的工作，採買須備妥日用品。

住宅需求以65分為中立，修正為 `round((全城平均－65)×0.6)`；空城中立65分。供水、衛生、污染與園景不再另重複加減需求，稅率及基本住房/工作壓力仍獨立作用。平均越好，在同樣空位、工作及道路條件下，遷入等候越短。

居民連續低於65分累計600遊戲秒後可能遷出；恢復至65分，在下一次人口評估清除該倒數。每15秒評估一次，每次最多遷出一人；原本無家90秒、失業180秒與缺日用品360秒規則仍在。倒數隨存檔保留，舊存檔缺少此欄位時從0開始，不補算離線時間。自由營造只顯示參考，不套用滿意度需求修正或強制遷出。這些權重與門檻是遊戲設計，並非歷史民生統計。

固定情境、真實遷入/遷出與Chrome分項介面證據見 [民生滿意驗收](evidence/wellbeing/README.md)。
...
等級能力、服務容量、範圍、升級費倍率與維護費統一由 `src/building-tiers.js` 計算。資訊卡同時顯示目前/下級外觀、能力與每日維護；經營模式確實扣升級費，自由營造免扣。五級不能再升，四格擴建是另一項操作。

| 能力 | 一級 | 二級 | 三級 | 四級 | 五級 |
|---|---:|---:|---:|---:|---:|
| 雅居住宅容量 | 4 | 5 | 6 | 7 | 8 |
| 四格住宅容量 | 8 | 10 | 12 | 14 | 16 |
| 精製作坊滿員加工倍率 | 1.5 | 1.875 | 2.25 | 2.625 | 3 |
| 四格作坊滿員加工倍率 | 2 | 2.5 | 3 | 3.5 | 4 |
| 商鋪店員名額 | 2 | 3 | 4 | 5 | 6 |
| 每位店員單件服務秒數 | 12 | 10 | 8 | 6 | 4 |
| 每格維護費倍率 | 1 | 2 | 3 | 4 | 5 |

普通初築民居一級容量2，首次升級轉為雅居、容量5；普通初築作坊一級基底1倍，首次升級為精製基底，再乘當級產能係數。工匠缺勤比例及學力仍作用於真正加工進度；升級不提供免費貨物。店鋪須有到場健康店員、顧客及存貨，實际交易間隔為表中秒數除以到場店員數；居民與外來顧客共用間隔，讀檔不重置。表格是服務能力，不是保證營業額。

供水、清運、巡守、醫療、教育與園景沿用各節的分級服務規則，全部改用同一能力表；實際覆蓋仍受道路、維護與人力限制。舊版 `level` 和四格基底保留，`tier` 現在同時影響功能。詳見 [五級功能驗收](evidence/tier-functions/README.md)。

## 園景宜居效益

園景對住宅的效益依門前步道路程計算：一級範圍24步、基礎8點，每升一級增加4步與2點；距離越遠效益越低，達範圍邊界或道路隔離時為0。未落成、受損園景不供效益，欠款依公共服務效率降低維護效果；水井、清運院與巡守所不當作休閒園景。

同一住宅接觸多處園景時，將原始效益由高到低排序，再依1、1/2、1/4等權重相加，總上限20。園景加成納入每位住戶的滿意環境分項，透過整體滿意度影響住宅需求；全城按住戶數加權（空屋權重1）的平均加成另提高有限人口吸引目標；安置與新入住候選依「污染減園景加成」排序，各自沿用原有資格條件；新住戶仍須符合工作、供水及道路要求。自由營造只顯示參考、不套用需求加成。

住宅小卡、園景小卡與收支總覽可查範圍、涵蓋住宅數與加成；效果由城市現況推導，不需新增存檔欄位。詳見 [園景驗收](evidence/gardens/README.md)。

## 工坊污染與住宅環境

工坊真正完成一件成品時才新增排放：陶器18、木器8、布匹14強度，向距離三格內的土地擴散並隨距離衰減。四格作坊將同一件產量的排放均分到四格源頭；產量越高，累積越多。各土地污染上限100；缺料、缺工、受損或成品庫滿而停工時不會憑空排放。上述為遊戲平衡數值，不是歷史排放測量。

停止生產後，污染每90遊戲秒減半。污染保存於土地，不會隨工坊搬移或拆除瞬間消失；讀檔保留殘留量，不補算離線時間。住宅以占地平均污染計算居住環境（100減污染）；城市污染摘要依各住宅住戶數加權，空屋權重1；需求改由民生滿意的環境分項統一計算，不另扣30點；新入住與安置優先選污染較低的可用住宅。

住宅、作坊小卡及收支總覽皆可查污染和居住環境。與每日清運的固體髒污分開處理，本階段不重複扣健康。自由營造保留污染數值但不排放、不衰減、不扣需求。詳見 [污染驗收](evidence/pollution/README.md)。
...
目前外觀已更新為「用途專屬升級」：24條建築路線分別增加庭院、工序設備、店面設施或園景。原V10共用的樓閣/牌樓/雙塔覆蓋已移除；不再要求所有建築靠逐級增高表現升級。舊存檔保留級數並直接套用新外觀。
...
|---|---|
| 1 初築 | 原有特色造型 |
| 2 添彩 | 第一項用途特色，如菜畦、晾坯架、藥櫃或荷花群 |
| 3 華庭 | 第二項設施，如拱窯、染布架、月門或曲橋 |
| 4 重簷 | 第三項特色，如煙囪群、織機、曬藥匾或花廊 |
| 5 盛景 | 專屬地標，如連窯、木構吊臂、藏書閣或荷心亭 |

升級保留原有占地、樣式、住戶與貨物；滿五級後停止升級，可復原上一步。原地升級與四格擴建分開，擴建、搬移及重新讀取都保留等級。四格花園合併取原花園最高等級。

沿用版本 8 存檔，新增 `tier` 外觀等級；舊存檔以一級起算，原有居住容量與加工能力保留。V9當時僅改外觀；目前開發版已接入分級功能與經營費用，以上方「五級建築經營能力」為準。尚在施工中的建築須先落成。驗證見 [V9 交接](docs/V9-HANDOFF.md)。
...
- **原地升級**：普通民居首次升為二級雅居，可住5人；工坊保留原品項，二級精製加工1.875倍。後續逐級提高，詳見上方能力表。
- **四格特殊建築**：一級四合雅宅可住8人；一級瓷窯大院、木作營造院、織錦大院可容6名工匠，加工2倍。擴建保留等級，高級容量與產能按能力表計算。可直接從圖錄選四格建造，或使用原房屋右下方的 2×2 四格擴建；須避開其他房屋與自建道路。空間不足時先搬移再擴建。
...
選取框只標示目前建築的實際占地，街坊格數另列文字。拆除提示明列此棟與占地，保留同街坊其他建築。所有對話框都連結唯一標題，作品視窗使用當前作品名。驗證見 `evidence/selection-dialogs/`。

## 手機與易讀排版

小螢幕從「營造與工具」展開建設、城市清單及存檔管理；選擇營造後收合工具，仍顯示占地預覽與取消。資訊小卡使用獨立可捲動面板；「文字」按鈕可在100%、150%、200%間切換，放大倍率相對於瀏覽器預設字級。紙底不透明，正文至少14px、核心標籤至少12px（以預設16px計）。實測與對比數值見 `evidence/readable-ui/`。

### 城鎮旅程

從「營造與工具 → 城鎮旅程」開啟。可選安居、工藝、文風願景，查看實際條件；自願開啟四步引導，主動觀察居民並增添作坊，再回看結果。住宅、工藝及文化里程碑各有一枚紀念章，符合條件才能領取且不重複發放。短程目標會事先說明操作與遊戲時間等待。可略過、重開或關閉挑戰，保留城市與已有進度；此設定獨立於金庫的城市經營／自由營造模式。

### 居民委託

在居民資訊卡按「回應這位居民的委託」，或從城鎮旅程查看。開啟引導挑戰後，可接取茶坊小聚或園中歇腳；場所需在住處六格內、道路相通且已落成。符合條件後按「回覆居民完成」，居民卡會留下回應及所採解法。可婉拒、放下、換解法或重新接取，沒有期限、扣分或清除建設；最多保留64份具名紀錄。

### 建築使用與營造成果

城市經營＋引導挑戰同時開啟時，升級與擴建需先符合資訊卡上的入住、到場工作或服務使用條件；自由營造保留直接升級。每張營造藍圖新增活動、居民行為與相關委託預覽。城鎮旅程的「營造成果對照」保存最近一次變動建物的營造前快照，重開時比較目前在場居民與實際活動；只描述兩個時間點，不宣稱所有差異都是改建造成。

### 私房地圖與命名

從「營造與工具 → 私房地圖」命名城鎮；建物資訊卡的「命名與收藏」可命名地標並加入書籤。名稱1–24字，保留建築原型名稱及遊戲品牌。最多收藏16處，書籤跟隨搬移後的位置，拆除後明示失效；移除書籤不拆建物。自訂名稱及書籤隨城鎮存檔保留。

### 我的文學選集

「營造與工具 → 我的文學選集」可查看九篇作品或篇章。親自開啟全文或節錄才留下讀訪印記，文人自動落筆不代替閱讀。同一篇由不同入口開啟不重複計數。作品卡可留400字玩家短箋、選一處個人地景；有來源對應的作品另可進行地景配對，錯配可重試，不宣稱史實原址。選三篇不同已讀作品、填入標題即可編成選集，也可隨時重編。

### 關注居民與熟識

居民資訊卡新增熟識紀錄與「觀察這段日常」。開卡或主動觀察不同活動才累積，重複跟隨不刷分，完成該居民委託另計一次；不要求每日登入，離線不衰退。可關注16位居民，從「營造與工具 → 關注居民」查看近況、開卡或跟隨。取消關注保留熟識；居民離鎮則保留最近已知行程並停用跳轉。

### 城鎮議事與探索冊

城鎮旅程新增園景、作坊兩項自願提案，缺少對應設施時可採納或擱置，落成且未受損後才能回應成果。活動探索冊列出說書、新貨、茶敘的實際出現條件；在活動進行時從「市井見聞」按「去街口看看」才留下初見紀錄。兩者均隨城鎮存檔保存，不設期限或扣款。

### 街頭往事與說書分支

說書依實際場地分成瓦舍「勾欄長篇評話」（28遊戲秒）與商鋪「街口短篇趣談」（12遊戲秒）。市井見聞的「街頭往事」保存最近24場圖文摘要、場地原因與實際到場居民。散場後可補訪歷史地點，不重演活動、不冒充親眼見過；集合取消也明確標示。

### 本次旅程回顧

城鎮旅程最上方彙整本次開啟後新增的紀念章、首次讀訪作品與完成委託，各列一次。重看不增加紀錄；一般營造保留本次起點，重新整理、重置或讀取另一存檔重新起算。可以按「結束本次觀察並暫停」休息，城鎮與收藏保留，按「繼續時間」即可接續。

### 用途專屬五級外觀（2026-09-16）

十款特色民居、工坊與商鋪各有獨立升級路線；茶坊、酒樓、園景與公設亦按用途分流。全部圖錄47種占地版本，各有五個不同模型；升級費用、能力與存檔契約不變。下級資訊顯示實際新增特色。

開發對照頁：`/tests/tier-identity-gallery.html`，可切換類型與一格/四格，同一列固定鏡頭比例。原「每級必須增高、五級翻倍」的視覺測試已依使用者新要求替換為每級不同幾何與用途特徵；未改動經營能力驗證。
