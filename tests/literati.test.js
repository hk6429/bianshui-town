...
test('v5 migration and v6 reload preserve freight, literature and road changes do not strand authors',()=>{
 ...
 setWeather(t,'clear');run(t,180);const data=JSON.parse(JSON.stringify(t)),restored=Town.restore(data);assert.equal(restored.toJSON().version,7);assert.deepEqual(restored.literati.collected,t.literati.collected);restored.place('home',[{x:-8,z:6}],true);run(restored,100);for(const a of restored.literati.actors)for(const p of a.route)assert(restored.roads.has(key(...p)));
});
