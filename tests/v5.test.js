...
test('v4 restores unchanged IDs and freight; v5 round trip preserves all blueprint footprints',()=>{
 ...
 const restored=Town.restore(JSON.parse(JSON.stringify(n)));assert.equal(restored.toJSON().version,5);assert.deepEqual(restored.buildings,n.buildings);assert.deepEqual([...restored.roads],[...n.roads]);
});
