...
test('daily upkeep depends on active facilities, is independent of tick sizes and settles once',()=>{
 ...
 const a=run(.25),b=run(1);assert.equal(a.city.maintenancePaid,b.city.maintenancePaid);assert.equal(a.city.maintenancePaid,900);const before=a.city.treasury;settleBudget(a);assert.equal(a.city.treasury,before);
...
test('only housed residents and new actual sales pay tax; policy range and tax pressure are bounded',()=>{
 const t=new Town({mode:'managed'});t.place('home',[{x:0,z:0}],true);assert.equal(householdTax(t),0);addResident(t,t.buildings[0]);addResident(t,t.buildings[0]);assert.equal(householdTax(t),600);
 const lot=t.economy.lots[0];lot.good='cloth';const funds=t.city.treasury;assert.equal(transfer(t,'boat','sold',1,'cloth'),1);assert.equal(t.city.treasury,funds+600);assert.equal(transfer(t,'sold','sold',1,'cloth'),0);assert.equal(t.city.treasury,funds+600);
 const demand=taxDemand(t);assert(setCityPolicy(t,{taxRate:20}));assert.equal(householdTax(t),1200);assert(taxDemand(t)<demand);const next=t.economy.lots.find(l=>l.at==='boat');next.good='cloth';const highFunds=t.city.treasury;assert.equal(transfer(t,'boat','sold',1,'cloth'),1);assert.equal(t.city.treasury,highFunds+1200);const before=JSON.stringify(t);assert(!setCityPolicy(t,{taxRate:21}));assert.equal(JSON.stringify(t),before);
});
