...
const ranked=(o)=>{const t=new Town(o);t.city.rank=MAX_RANK;t.journey.literary=Object.fromEntries(Object.keys(LITERARY_QUESTS).map(id=>[id,{step:4,note:''}]));layRoad(t,'lane',[{x:-1,z:0}]);return t;};   // 先有路才能蓋
test('preview names and counts match actual placement for every design and supported footprint',()=>{
 for(const [design,d] of Object.entries(DESIGNS))for(const size of d.sizes){const t=ranked(),cells=size===4?squareCells({x:design==='yueyangTower'?1:0,z:0}):at(),before=JSON.stringify(t),r=previewPlan(t,{mode:d.type,design},cells);assert(r.valid,`${design}: ${r.reason}`);assert.equal(JSON.stringify(t),before,'preview has no side effects');assert(t.place(d.type,cells,true,design));for(const b of t.buildings)assert(r.result.includes(b.name),`${design}: ${r.result} != ${b.name}`);assert.equal(t.buildings.length,1);}
});
test('same residence blueprint predicts one four-cell mansion or four distinct houses in a line',()=>{
 for(const [cells,count,name] of [[squareCells({x:0,z:0}),1,'四合雅宅'],[[{x:-3,z:0},{x:-2,z:0},{x:-1,z:0},{x:0,z:0}],4,'雅居小樓']]){const t=new Town({mode:'managed'});layRoad(t,'lane',[cells.length===4&&name==='四合雅宅'?{x:2,z:0}:{x:1,z:0}]);t.city.treasury=300000;const r=previewPlan(t,{mode:'home',design:'residence'},cells);assert(r.valid);assert(r.result.includes(name));assert(r.result.includes(`占地4格 · ${count}棟`));assert.equal(r.cost,72000);assert(t.place('home',cells,false,'residence'));assert.equal(t.buildings.length,count);assert.equal(t.city.treasury,228000);assert(t.buildings.every(b=>b.name===name));}
});
test('previews distinguish occupied houses, roads, edges, disconnected shapes, footprint restrictions and insufficient funds',()=>{
 const t=new Town({mode:'managed'});t.place('home',at(),true,'residence');layRoad(t,'lane',at(1));layRoad(t,'lane',at(-4));
...
test('沒有臨街的建築不能蓋，提示要先鋪路',()=>{
 const t=new Town({mode:'managed'});
 const far=previewPlan(t,{mode:'home',design:'residence'},[{x:-3,z:-3}]);
 assert(!far.valid);assert.match(far.reason,/旁邊沒有路/);
 assert(layRoad(t,'lane',[{x:-3,z:-2}]));
 const near=previewPlan(t,{mode:'home',design:'residence'},[{x:-3,z:-3}]);
