...
import {previewPlan} from '../src/plan-preview.js';import {layRoad} from '../src/urban.js';
const at=(x=0,z=0)=>[{x,z}];
test('preview names and counts match actual placement for every design and supported footprint',()=>{
 for(const [design,d] of Object.entries(DESIGNS))for(const size of d.sizes){const t=new Town(),cells=size===4?squareCells({x:0,z:0}):at(),before=JSON.stringify(t),r=previewPlan(t,{mode:d.type,design},cells);assert(r.valid,`${design}: ${r.reason}`);assert.equal(JSON.stringify(t),before,'preview has no side effects');assert(t.place(d.type,cells,true,design));for(const b of t.buildings)assert(r.result.includes(b.name),`${design}: ${r.result} != ${b.name}`);assert.equal(t.buildings.length,1);}
});
test('same residence blueprint predicts one four-cell mansion or four distinct houses in a line',()=>{
 for(const [cells,count,name] of [[squareCells({x:0,z:0}),1,'四合雅宅'],[[{x:-3,z:0},{x:-2,z:0},{x:-1,z:0},{x:0,z:0}],4,'雅居小樓']]){const t=new Town({mode:'managed'}),r=previewPlan(t,{mode:'home',design:'residence'},cells);assert(r.valid);assert(r.result.includes(name));assert(r.result.includes(`占地4格 · ${count}棟`));assert.equal(r.cost,480);assert(t.place('home',cells,false,'residence'));assert.equal(t.buildings.length,count);assert.equal(t.city.treasury,1920);assert(t.buildings.every(b=>b.name===name));}
});
test('previews distinguish occupied houses, roads, edges, disconnected shapes, footprint restrictions and insufficient funds',()=>{
 ...
 t.city.treasury=0;const r=previewPlan(t,{mode:'home'},at(-3));assert(!r.valid);assert.match(r.reason,/需要120文，目前0文/);assert(!t.place('home',at(-3)));
});
test('road squares and auto-merged gardens predict the actual combined result',()=>{
 ...
 const road=new Town(),cells=squareCells({x:0,z:0}),p=previewPlan(road,{mode:'road',editing:{kind:'road',type:'lane'}},cells);assert(p.valid);assert.match(p.result,/新成1處街坊廣場/);assert(layRoad(road,'lane',cells));
});
