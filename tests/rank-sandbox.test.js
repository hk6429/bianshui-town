...
import {buyLand} from '../src/city-finance.js';
...
test('自由營造可以免費買地，城市經營照價付錢',()=>{
 const t=new Town({mode:'sandbox'});
 assert.equal(buyLand(t),true,'自由營造應該能擴地');
 assert.equal(t.city.expansion,1);
 const m=new Town({mode:'managed'});
 m.city.treasury=0;
 assert.equal(buyLand(m),false,'城市經營沒錢不能買地');
 m.city.treasury=1e8;
 assert.equal(buyLand(m),true);
});
