...
import {setJourneyMode,setUseGate} from '../src/journey.js';
...
test('challenge rejects unused home without charge, allows actual households; sandbox and disabled challenge retain free upgrades',()=>{
 const t=town(),b=t.buildings[0];t.city.mode='managed';setUseGate(t,true);const money=t.city.treasury;assert.equal(upgradeUse(t,b).allowed,false);assert.equal(upgradeBuilding(t,b.id),false);assert.equal(t.city.treasury,money);assert.match(upgradeUse(t,b).text,/入住/);
 ...
 const empty=town();setUseGate(empty,true);assert(upgradeBuilding(empty,empty.buildings[0].id));empty.city.mode='managed';empty.journey.useGate=false;assert(upgradeBuilding(empty,empty.buildings[0].id));
});
test('work requires an actual present healthy worker, not merely a job assignment; garden requires presence',()=>{
 const t=town();t.tick(.05);t.place('work',[{x:1,z:0}],true);t.place('garden',[{x:0,z:1}],true,'garden');t.city.mode='managed';setUseGate(t,true);const work=t.buildings.find(b=>b.type==='work'),garden=t.buildings.find(b=>b.type==='garden'),p=t.people[0];p.work=work.id;p.outside=true;
...
test('tea visitors in the real simulation perform the previewed activity; utility use comes from occupied service coverage',()=>{
 ...
 t.place('garden',[{x:0,z:1}],true,'well');const well=t.buildings.find(b=>b.design==='well');t.city.mode='managed';setUseGate(t,true);assert(upgradeUse(t,well).allowed);t.people=[];assert.equal(upgradeUse(t,well).allowed,false);
});
