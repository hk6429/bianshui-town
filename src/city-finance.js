...
import {transitTaxRate,wineExcise} from './civic.js';
...
export const createCity=(mode='sandbox',time=8)=>({version:2,mode,logisticsLevel:1,trade:createTrade(),treasury:300000,expansion:0,taxRate:10,day:Math.floor(time/24),sanitationDay:Math.floor(time/24),fireDay:Math.floor(time/24),ledger:[],taxIncome:0,spent:0,maintenancePaid:0,deficitDays:0});
...
export function settleBudget(t){const c=t.city,day=Math.floor(t.time/24);if(day<=c.day)return;if(!managed(t)){c.day=day;return;}const days=day-c.day,income=(householdTax(t)+wineExcise(t))*days,expense=dailyUpkeep(t)*days,before=c.treasury,net=(income-expense)/days;c.day=day;c.treasury+=income-expense;c.taxIncome+=income;c.maintenancePaid+=expense;c.deficitDays=c.treasury>=0?0:before<0?c.deficitDays+days:days-Math.floor(before/-net);record(t,`${days} 日住稅、榷酤與維護費`,income-expense);}
