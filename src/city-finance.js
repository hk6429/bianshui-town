...
export function settleBudget(t){const c=t.city,day=Math.floor(t.time/24);if(day<=c.day)return;if(!managed(t)){c.day=day;return;}const days=day-c.day,income=householdTax(t)*days,expense=dailyUpkeep(t)*days;c.day=day;c.treasury+=income-expense;c.taxIncome+=income;c.maintenancePaid+=expense;c.deficitDays=c.treasury<0?c.deficitDays+days:0;record(t,`${days} 日人口稅 ${income}／維護費 ${expense}`,income-expense);}
export function setCityPolicy(t,{mode=t.city.mode,taxRate=t.city.taxRate}){if(!['managed','sandbox'].includes(mode)||!Number.isInteger(taxRate)||taxRate<0||taxRate>20)return false;t.city.mode=mode;t.city.taxRate=taxRate;t.city.day=Math.floor(t.time/24);return true;}
