...
export class Soundscape {
 ...
 async toggle(){
  if(!this.ctx){const Audio=window.AudioContext||window.webkitAudioContext;if(!Audio)throw new Error('目前瀏覽器不支援環境音');this.ctx=new Audio();this.master=this.ctx.createGain();this.master.gain.value=0;this.master.connect(this.ctx.destination);
   ...
   this.water=this.ctx.createBiquadFilter();this.water.type='lowpass';this.water.frequency.value=850;const gain=this.ctx.createGain();gain.gain.value=.2;source.connect(this.water).connect(gain).connect(this.master);source.start();
  }
