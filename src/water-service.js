...
export const isUtility=b=>b.design==='well';
...
export function waterReport(t){
 ...
 function assign(slot,seen=new Set()){
  for(const w of slot.choices){if(seen.has(w.id))continue;seen.add(w.id);const list=users.get(w.id);
   if(list.length<waterCapacity(w)){list.push(slot);return true;}
...
export function waterStatus(t,b,report=waterReport(t)){
 ...
 if(isUtility(b)){const w=report.wells.find(w=>w.id===b.id);return w?`水井容量 ${w.capacity} 人；正在供水 ${w.used} 人，預留住宅空位 ${w.reserved} 席；沿路服務距離 ${w.range} 步。`:'水井施工中，落成後開始供水。';}
 return '';
}
