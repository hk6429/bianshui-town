...
export const fireActionCost=b=>b.fireWarningAt!==undefined?20:damaged(b)?Math.ceil(b.fireDamage*(b.footprint?.length||1)):0;
...
export function fireStatus(t,b){
 ...
 if(b.fireWarningAt!==undefined)return `火警預警：${Math.max(0,Math.ceil(b.fireWarningAt-t.elapsed))} 秒內處置；可付20文排除，或增設可達巡守所減輕損害。`;
 if(damaged(b))return `受損 ${b.fireDamage}／100，營業與生產暫停；${Math.max(0,Math.ceil(b.fireRepairAt-t.elapsed))} 秒後免費整修完成，或付 ${fireActionCost(b)} 文立即修復。住戶、貨物與建築保留。`;
