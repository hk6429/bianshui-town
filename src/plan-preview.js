...
import {DESIGNS,gardenMergeGroups} from './heritage.js';
...
export function previewPlan(t,{mode,design=null,editing=null},cells){
 ...
 if(editing?.kind==='move'){
  ...
  else{cost=roadCost(t,editing.type,cells);const roads=t.publicWorks.filter(p=>!cells.some(c=>c.x===p.x&&c.z===p.z)).concat(cells.map(c=>({...c,type:editing.type}))),added=publicSquares({...t,publicWorks:roads}).length-publicSquares(t).length;result=`${ROAD_TYPES[editing.type]} · 占地${cells.length}格${added>0?` · 新成${added}處街坊廣場`:''}`;}
 }else{
  reason=placementIssue(t,cells);const plan=constructionPlan(mode,cells,design,t.nextId);reason=reason||plan.reason;cost=buildCost(mode,cells);
  if(!plan.reason){const newIds=new Set(plan.buildings.map(b=>b.id)),merges=gardenMergeGroups([...t.buildings,...plan.buildings]).filter(parts=>parts.some(b=>newIds.has(b.id))),mergedIds=new Set(merges.flat().map(b=>b.id)),names=plan.buildings.filter(b=>!mergedIds.has(b.id)).map(b=>b.name).concat(merges.map(()=>DESIGNS.scholarGarden.name)),existing=merges.flat().filter(b=>!newIds.has(b.id)).length;
   result=`${[...new Set(names)].join('、')} · 占地${cells.length}格 · ${names.length}${mode==='garden'?'處園景':'棟'}${existing?`（另合併既有${existing}格花園）`:''}`;
  }
 }
 if(!affordable(t,cost))reason=reason||`金庫不足：需要${cost}文，目前${t.city.treasury}文`;
 const price=managed(t)?`本次${cost}文 · 金庫${t.city.treasury}文`:'自由營造 · 本次不扣款';
