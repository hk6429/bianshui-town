...
export function previewPlan(t,{mode,design=null,editing=null},cells){
 ...
 if(editing?.kind==='move'){
  ...
  reason=!b?'原建築已不存在':b.stage<3?'建築尚未落成，不能搬移':placementIssue(t,cells,{ignore:b.id})||landmarkPlacementIssue(t,b.design,cells);
  ...
  reason=placementIssue(t,cells);const plan=constructionPlan(mode,cells,design,t.nextId);reason=reason||plan.reason||lockedReason(t,plan.design)||landmarkPlacementIssue(t,plan.design,cells);
  // 先有路才有街：沒有臨街的建築，人和貨都到不了。
  if(!reason&&managed(t)&&!touchesRoad(t,cells))reason='旁邊沒有路：先鋪一條路連過來，人和車才走得到';cost=buildCost(mode,cells,plan.design);
