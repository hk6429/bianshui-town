...
export function installLiteraryQuestsUI({getTown,change,build}){
 ...
 let drafts={};
 const adventure=createAdventureUI({getTown,change,rerender:message=>render(message)});
 dialog.addEventListener('input',adventure.input);
 ...
 dialog.addEventListener('click',event=>{
  if(adventure.click(event)||guideClick(event,dialog)||learning.click(event))return;
