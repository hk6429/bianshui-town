...
export const newLearningQuest=()=>({attempts:[],hints:[0,0,0],revisions:[],reviews:[],prediction:null});
...
export function recordLearningAnswer(t,id,{step,answer,evidence,reason,at=Date.now()}){
 ...
 const state=ensure(t,id),correct=answer===q.steps[step].answer,evidenceMatch=evidenceLines(id)[evidence].step===step;
 ...
 if(correct&&evidenceMatch)answerQuest(t,id,step,answer);
 return true;
}
export function saveLearningRevision(t,id,{text,feedback='',changes='',at=Date.now()}){
 ...
 const state=ensure(t,id);state.revisions.push({text:text.trim(),feedback:feedback.trim(),changes:changes.trim(),at});state.revisions=state.revisions.slice(-12);return true;
}
export function recordPrediction(t,id,{choice,reason,observation='',at=Date.now()}){
 ...
 const state=ensure(t,id),old=state.prediction;
...
export function reviewDue(t,id,now=Date.now()){
 ...
 if(!first)return false;return now>=(last?.at||first.at)+DAY;
}
export function recordTransfer(t,id,{variant,answer,reason,at=Date.now()}){
 const task=LEARNING_CONTENT[id]?.transfer[variant],s=learningQuest(t,id);if(!task||!Number.isInteger(variant)||!Number.isInteger(answer)||answer<0||answer>=task.choices.length||typeof reason!=='string'||!reason.trim()||reason.length>400||!timestamp(at))return false;
 const due=reviewDue(t,id,at);ensure(t,id).reviews=[...s.reviews,{variant,answer,reason:reason.trim(),correct:answer===task.answer,delayed:due,at}].slice(-12);return true;
}
export function learningSummary(t,id){
 const s=learningQuest(t,id),passed=s.attempts.filter(a=>a.correct&&a.evidenceMatch),independent=passed.filter(a=>!a.hints),latest=s.reviews.at(-1);
...
export function validateLearning(value,path='journey.learning'){
 ...
 for(const [id,s]of Object.entries(value.quests)){
  object(s,['attempts','hints','revisions','reviews','prediction']);list(s.attempts,90);list(s.revisions,12);list(s.reviews,12);list(s.hints,3);if(s.hints.length!==3||s.hints.some(h=>!integer(h,3)))fail('提示層級錯誤');
  ...
  for(const r of s.reviews){object(r,['variant','answer','reason','correct','delayed','at']);text(r.reason);const q=LEARNING_CONTENT[id].transfer[r.variant];if(!integer(r.variant,1)||!integer(r.answer,2)||!q||!r.reason.trim()||r.correct!==(q.answer===r.answer)||typeof r.delayed!=='boolean'||!timestamp(r.at))fail('遷移紀錄無效');}
