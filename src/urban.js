...
export function upgradeBuilding(t,id,expand=false){
 ...
 if(expand){if(!['home','work'].includes(b.type)||b.footprint)return false;const cells=squareCells({x:b.x,z:b.z});if(!freeCells(t,cells,id))return false;detach(t,b);attach(t,b,cells);}
 else{if(tierOf(b)>=MAX_TIER)return false;b.tier=tierOf(b)+1;}
