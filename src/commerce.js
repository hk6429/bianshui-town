...
export const shopStatus=(t,b)=>`${shopIsOpen(t,b)?'營業中':'暫停營業'} · 到場店員 ${presentWorkers(t,b).length}／${jobCapacity(b)} 人 · 病假 ${t.workers(b).filter(p=>onSickLeave(t,p)).length} 人 · 招工 ${Math.max(0,jobCapacity(b)-t.workers(b).length)} 人`;
