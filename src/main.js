...
$('#life-btn').onclick=()=>{journalOpen=!journalOpen;$('#life-btn').setAttribute('aria-expanded',String(journalOpen));updateJournal();};
$('#life-panel').onclick=e=>{const btn=e.target.closest('[data-focus]');if(!btn)return;const where={bridge:[18,-16],dock:[12,8],town:[-7,0]}[btn.dataset.focus];const delta=new THREE.Vector3(where[0],0,where[1]).sub(scene.controls.target);scene.camera.position.add(delta);scene.controls.target.set(where[0],0,where[1]);scene.camera.zoom=1.8;scene.camera.updateProjectionMatrix();if(innerWidth<600){journalOpen=false;updateJournal();}};
...
$('#confirm-new').onclick=()=>{town=new Town();scene.reset();scene.resetView();pinned=hovered=null;journalOpen=false;updateJournal();$('#inspector').hidden=true;$('#welcome').hidden=false;$('#confirm-reset').close();$('#help').close();setMode('explore');save();};
...
function renderInspector(){
 ...
 if(ref.kind==='building'){
  ...
  const a=[...town.life.visitors,...town.life.porters,...town.life.oxen].find(a=>a.id===ref.id);if(!a){panel.hidden=true;return;}panel.innerHTML=`<div class="eyebrow">${({ox:'牛車運送',porter:'碼頭腳夫',peddler:'挑擔行商',traveler:'過橋旅人',shopper:'趕集來客'})[a.kind]}</div><h2>${escape(a.name)}</h2><p>${escape(a.action)}</p><hr><label>此刻攜帶</label><p>${a.kind==='peddler'?'挑擔與日用雜貨':a.carrying?`${a.carrying} 件貨物`:'輕裝行走'}</p><label>行程</label><p>${a.walking?'正在沿街道前往目的地':'停留、歇腳或等候'}</p>`;if(outlined){scene.clearGroup(scene.selection);outlined='';}
 }else if(ref.kind==='boat'){
  const b=town.life.boat;panel.innerHTML=`<div class="eyebrow">汴河水運 · 第 ${b.trips+1} 航次</div><h2>汴河漕船</h2><p>${boatStatus()}</p><hr><label>船上貨物</label><p>${b.cargo} 件，${b.mast?'桅杆升起':'已收桅，準備過橋'}</p><label>本鎮累計</label><p>已卸 ${town.life.dock.received} 件 · 已送商鋪 ${town.life.dock.delivered} 件</p>`;if(outlined){scene.clearGroup(scene.selection);outlined='';}
 }else{
...
window.__townDebug={snapshot:()=>JSON.parse(JSON.stringify(town)),screenCell:(x,z)=>{const p=new THREE.Vector3(x*4,0,z*4).project(scene.camera);return {x:(p.x+1)/2*innerWidth,y:(1-p.y)/2*innerHeight};},screenBuilding:id=>{const b=town.building(id);if(!b)return null;const p=new THREE.Vector3(b.x*4,1,b.z*4).project(scene.camera);return {x:(p.x+1)/2*innerWidth,y:(1-p.y)/2*innerHeight};},renderInfo:()=>({...scene.renderer.info.render}),mode:()=>mode,sound:()=>({enabled:sound.enabled,state:sound.ctx?.state||'not-started',gain:sound.master?.gain.value||0,volume:sound.volume}),screenLife:id=>{const a=[...town.life.visitors,...town.life.porters,...town.life.oxen].find(a=>a.id===id);if(!a)return null;const p=new THREE.Vector3(a.x,1,a.z).project(scene.camera);return {x:(p.x+1)/2*innerWidth,y:(1-p.y)/2*innerHeight};}};
