...
function applyUrban(fn,{record=true}={}){
 ...
 const result=editTown(town,draft=>{if(editFault)draft.rebuildRoads=()=>{throw Error('測試：城市道路更新中斷');};const edited=fn(draft);if(edited&&record)recordConstruction(town,draft);return edited;},{prepare:draft=>{scene.reset();scene.update(draft,0);},persist:data=>fixtureMode?{ok:true}:saveStore.save(data)});
