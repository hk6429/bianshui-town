...
function renderInspector(){
 ...
 if(ref.kind==='building'){
  ...
  const s=marketStalls(town).find(s=>s.id===ref.id);if(!s){panel.hidden=true;return;}inspectorContent(panel,stallDetails(s,town.market.trades,marketOpen(town)));if(outlined){scene.clearGroup(scene.selection);outlined='';}
...
function buildingTools(b){if(b.stage<3)return '';const tier=tierOf(b);return `<hr><label>建築 ${tier}／${MAX_TIER} 級 · ${TIER_NAMES[tier-1]}${b.type==='home'?` · 可住 ${b.footprint?8:b.level>=2?4:2} 人`:b.type==='work'?` · 加工 ${b.footprint?'2':b.level>=2?'1.5':'1'} 倍`:''}</label><p class="tier-detail">${tier<MAX_TIER?`下級：${TIER_DETAILS[tier]}`:'已達五級 · 盛景落成'}</p><div class="building-tools"><button data-manage="move" data-building="${escape(b.id)}">搬移</button><button data-manage="delete" data-building="${escape(b.id)}">拆除</button>${tier<MAX_TIER?`<button data-manage="upgrade" data-building="${escape(b.id)}">升至 ${tier+1} 級</button>`:''}${['home','work'].includes(b.type)&&!b.footprint?`<button data-manage="expand" data-building="${escape(b.id)}">四格擴建 ↘</button>`:''}</div>`;}
...
$('#stalls-btn').onclick=()=>{const stalls=marketStalls(town);$('#stalls-list').innerHTML=stalls.length?stalls.map(stallListItem).join(''):'<p>先讓商鋪或說書棚落成，或將道路鋪成 2 × 2 廣場，攤販便會出現。</p>';$('#public-works').close();$('#stalls').showModal();};
