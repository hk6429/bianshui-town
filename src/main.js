...
$('#demo-btn').onclick=()=>{if(!applyUrban(draft=>{draft.demo();return true;}))return;$('#welcome').hidden=true;setMode('explore');save();toast('歡迎來到汴水小鎮，點選屋舍看看誰在裡面');};
...
canvas.addEventListener('pointerup',e=>{if(e.button!==0)return;if(drag&&editing){const op=editing,cells=drag.cells,squareCount=publicSquares(town).length;const success=applyUrban(draft=>op.kind==='move'?moveBuilding(draft,op.id,cells[0]):op.type==='erase'?removeRoad(draft,cells):layRoad(draft,op.type,cells));toast(success?(op.kind==='move'?'建築已搬移，住戶與貨物保留':publicSquares(town).length>squareCount?'四格道路已合成街坊廣場；可按復原撤銷':'公共道路已更新；可按復原撤銷'):editFailure||'這裡無法操作，請避開建築及範圍外地面');drag=null;scene.clearGroup(scene.preview);if(success&&op.kind==='move'){setMode('explore');pinned={kind:'building',id:op.id};}down=null;return;}if(drag){const placed=applyUrban(draft=>draft.place(mode,drag.cells,false,selectedDesign));if(placed){$('#welcome').hidden=true;const b=town.buildings.find(b=>b.blockId===placed.id);toast(`${b.footprint?'四格合建 · ':''}${b.name}開始${b.design==='pond'?'挖池':'動工'}`);save();if(selectedDesign){setMode('explore');pinned={kind:'building',id:b.id};hovered=null;}}else toast(editFailure||'這裡放不下，請選擇範圍內的空地；四格須完整空出 2 × 2');drag=null;scene.clearGroup(scene.preview);}else if(down&&Math.hypot(e.clientX-down.x,e.clientY-down.y)<5){pinned=scene.pick(e.clientX,e.clientY);hovered=pinned;}down=null;});
...
function applyUrban(fn){
 ...
 const result=editTown(town,draft=>{if(editFault)draft.rebuildRoads=()=>{throw Error('測試：城市道路更新中斷');};return fn(draft);},{prepare:draft=>{scene.reset();scene.update(draft,0);},persist:data=>fixtureMode?{ok:true}:saveStore.save(data)});
 ...
 town=result.town;undoTown=result.before;runtime.reset();recovery=new RecoveryPoint({read:()=>town.toJSON(),validate:validateSave});$('#undo-urban').hidden=false;$('#save-status').textContent=fixtureMode?'預覽場景 · 不覆寫小鎮':'進度已留存';return result.result;
}
