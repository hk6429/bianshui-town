...
const $=s=>document.querySelector(s),canvas=$('#world'),sound=new Soundscape();let journalOpen=false,selectedDesign=null,plotSize=1;
...
const fixtureName=new URLSearchParams(location.search).get('fixture');const fixtureMode=import.meta.env.DEV&&['v4','v5','v6'].includes(fixtureName);
...
canvas.addEventListener('pointerdown',e=>{stopFollowing();if(e.button!==0)return;down={x:e.clientX,y:e.clientY};if(mode!=='explore'){const c=cellAt(e);if(c){const cells=plannedCells(c);drag={start:c,cells};scene.outline(cells,town.canPlace(cells)?0x668856:0xb5644c);canvas.setPointerCapture(e.pointerId);}}});
canvas.addEventListener('pointermove',e=>{if(mode!=='explore'){const c=cellAt(e);if(!c)return;const cells=selectedDesign?plannedCells(c):drag?dragCells(drag.start,c):[c];if(drag)drag.cells=cells;scene.outline(cells,town.canPlace(cells)?0x668856:0xb5644c);return;}if(!down)hovered=scene.pick(e.clientX,e.clientY);});
canvas.addEventListener('pointerup',e=>{if(e.button!==0)return;if(drag){const placed=town.place(mode,drag.cells,false,selectedDesign);if(placed){$('#welcome').hidden=true;const b=town.buildings.find(b=>b.blockId===placed.id);toast(`${b.footprint?'四格合建 · ':''}${selectedDesign?DESIGNS[selectedDesign].name:TYPES[mode]}開始${b.design==='pond'?'挖池':'動工'}`);save();if(selectedDesign){setMode('explore');pinned={kind:'building',id:b.id};hovered=null;}}else toast('這裡放不下，請選擇範圍內的空地；四格須完整空出 2 × 2');drag=null;scene.clearGroup(scene.preview);}else if(down&&Math.hypot(e.clientX-down.x,e.clientY-down.y)<5){pinned=scene.pick(e.clientX,e.clientY);hovered=pinned;}down=null;});
...
function plannedCells(c){return selectedDesign&&plotSize===4?squareCells(c):[c];}
function literatureLink(b){const d=DESIGNS[designFor(b)];if(!d)return '';const source=SOURCES[d.source];return `<section class="literary-note"><small>${escape(source.author)} · ${escape(source.title)}</small><blockquote>${escape(source.quote)}</blockquote><button class="tag" data-reading="${d.source}">讀作品與地景 ↗</button></section>`;}
