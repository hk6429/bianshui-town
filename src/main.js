...
import {genericArt} from './plan-art.js';
...
$('#inspector').onclick=e=>{
 if(handleLandmarkAction(e,{change:changeJourney,toast,render:renderInspector}))return;
...
function renderProduction(){
 const overview=RECIPES.map(r=>{const works=town.buildings.filter(b=>isWorkshop(b)&&recipeFor(b)===r);return `<article class="production-card"><h3>${(r.inputs||[r.input]).map(g=>GOODS[g]).join('＋')} → ${GOODS[r.output]}</h3><p>原料 ${(r.inputs||[r.input]).map(g=>`${GOODS[g]} ${town.economy.lots.filter(l=>l.good===g&&!['sold','consumed'].includes(l.at)).length} 份`).join('、')} · 成品 ${town.economy.lots.filter(l=>l.good===r.output&&!['sold','consumed'].includes(l.at)).length} 件 · 售出 ${escape(town.economy.sold[r.output]||0)} 件</p><p>${works.length?works.map(b=>`${escape(b.name)}：${escape(b.productionStatus||'等候工匠與原料')}`).join('<br>'):'先安放對應作坊，再等工匠到來'}</p></article>`;}).join('');$('#production-overview').innerHTML=overview;
...
function renderLot(){
 ...
 $('#lot-detail').innerHTML=`<h3 id="lot-detail-title">${lotIdentity(l)}</h3><p>來源：${escape(l.origin)}${l.madeAt?` · 製作：${escape(town.building(l.madeAt)?.name)}`:''}</p><ol class="cargo-trail">${l.trailOmitted?`<li>較早的 ${escape(l.trailOmitted)} 筆紀錄已收進摘要；來源與製作紀錄仍保留。</li>`:''}${l.trail.map(step=>`<li>第 ${Math.floor(step.time/24)+1} 日 ${formatTime(step.time)} · ${escape(placeName(town,step.at))}</li>`).join('')}</ol>${l.materials?`<p>印製原料：${l.materials.map(m=>`${escape(GOODS[m.good])} #${m.id}（${escape(m.origin)}）`).join('、')}</p>`:''}${locateLot(town,l)?lotFocusButton(l):l.at==='consumed'?`<p>此份印墨已用於書籍 #${l.consumedBy}，未列為銷售。</p>`:'<p>這件貨已由來客購得。</p>'}`;
}
...
function renderBlueprints(){document.querySelectorAll('[data-design-filter]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.designFilter===designFilter)));document.querySelectorAll('[data-size]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.size)===plotSize)));$('#design-filter-note').textContent=designFilter==='all'?'共 '+Object.keys(DESIGNS).length+' 款圖樣，依民居／商鋪／作坊／園景／文教／公設／祠廟／水運分類。':designFilter==='new'?'第二批新增的十款外觀。':CATEGORY_NOTE[designFilter]||'';
 ...
 $('#blueprint-grid').innerHTML=Object.entries(DESIGNS).filter(([id,d])=>designFilter==='all'||designFilter==='new'&&!!NEW_DESIGNS[id]||designCategory(id)===designFilter).map(([id,d])=>`<article class="blueprint-card${built.get(id)?' is-built':''}">${planArt(id)}<div><h3>${d.name}<span class="blueprint-tag">${CATEGORIES[designCategory(id)]}</span></h3><p class="blueprint-state">${built.get(id)?`鎮上已有 ${built.get(id)} 處`:'尚未營造'} · 每格 ${formatMoney(cellCost(d.type,id))}</p><p>${d.detail}</p>${lockedReason(town,id)?`<p class="blueprint-lock">${escape(lockedReason(town,id))}</p>`:""}${blueprintLifeHTML(id)}<small>${d.source?SOURCES[d.source].author+' · '+SOURCES[d.source].title:'宋式街坊 · 遊戲建築設計'}</small></div><button data-plan="${id}">${d.sizes.includes(plotSize)?(plotSize===4?'四格合建':d.sizes.includes(4)?'點蓋／拖曳合建':'一格營造'):'使用'+(d.sizes.includes(4)?'四格合建':'一格營造')}${id==='pond'?' · 挖池塘':''} ↗</button></article>`).join('');}
$('#blueprints-btn').onclick=()=>{renderBlueprints();$('#blueprints').showModal();};
$('#blueprints').onclick=e=>{const filter=e.target.closest('[data-design-filter]');if(filter){designFilter=filter.dataset.designFilter;renderBlueprints();return;}const size=e.target.closest('[data-size]');if(size){plotSize=Number(size.dataset.size);renderBlueprints();return;}const plan=e.target.closest('[data-plan]');if(!plan)return;const d=DESIGNS[plan.dataset.plan];const lock=lockedReason(town,plan.dataset.plan);if(lock){toast(lock);return;}if(!d.sizes.includes(plotSize))plotSize=d.sizes[0];setMode(d.type);selectedDesign=plan.dataset.plan;pinned=hovered=null;journalOpen=false;updateJournal();$('#blueprint-status').textContent=`${d.name} · ${plotSize===4?'四格合建':'一格小築'}`;$('#blueprint-status').hidden=false;$('#mode-hint').textContent=`${d.name} · 點選${plotSize===4?'完整的 2 × 2':'一格'}空地${d.type==='garden'?'營造園景':'開始動工'} · Esc 取消`;$('#welcome').hidden=true;$('#blueprints').close();focusScene();};
...
$('#handbook-btn').onclick=()=>$('#handbook').showModal();
$('#close-handbook').onclick=()=>$('#handbook').close();
