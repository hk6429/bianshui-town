import {continuityView,recordContinuity,CONTINUITY_RESIDENTS} from './adventure-continuity.js';
...
export function createAdventureUI({getTown,change,rerender}){
 ...
 const drafts=new Map(),messages=new Map(),reflections=new Map();
 const roundKey=(id,round)=>`${id}:${round}`;
 function sync(){if(identity!==getTown()){identity=getTown();drafts.clear();messages.clear();reflections.clear();}}
 function render(id){
  sync();
  const v=adventureView(getTown(),id);if(!v)return '';
  const continuity=continuityView(getTown(),id),key=roundKey(id,v.round);
  const residents=id==='yueyang'?CONTINUITY_RESIDENTS:[];
  const reflection=reflections.get(key)||continuity?.record||{evidenceId:'',reason:''};
  const continuityPanel=continuity?.branchId?`<section class="adventure-continuity" aria-label="岳陽樓跨輪記憶"><h5>居民記得你上次的安排</h5><p class="adventure-new-clue"><strong>本輪新線索：${esc(continuity.title)}</strong></p><details data-adventure-continuity ${reflections.has(key)?'open':''}><summary>查看前輪記憶與新線索，回原文修正想法</summary><ul>${continuity.residents.map(r=>`<li><strong>${esc(r.name)}・${esc(r.role)}</strong><p>${esc(r.memory)}</p></li>`).join('')}</ul><p>${esc(continuity.clue)}</p><p>${esc(continuity.prompt)}</p><label for="adventure-evidence-${esc(id)}">選一句原文支持你的想法</label><select id="adventure-evidence-${esc(id)}" data-adventure-field="evidence" data-adventure-id="${esc(id)}"><option value="">請選擇原文</option>${continuity.evidenceOptions.map(o=>`<option value="${esc(o.id)}" ${reflection.evidenceId===o.id?'selected':''}>${esc(o.text)}</option>`).join('')}</select><label for="adventure-reflection-${esc(id)}">看到新線索後，我想修正或補充……（最多 600 字）</label><textarea id="adventure-reflection-${esc(id)}" data-adventure-field="reflection" data-adventure-id="${esc(id)}" maxlength="600" rows="3">${esc(reflection.reason||'')}</textarea><p data-adventure-reflection-draft>${reflections.has(key)?'修正草稿保留中；請按下儲存。':continuity.record?'最近一份修正已儲存。':'選擇原文並寫下理由後，即可儲存。'}</p><button ${action(id,'reflect')}>儲存這次修正</button><p class="adventure-note">不自動評定理解或理由對錯；留給你、同學與老師討論。只保留本輪最近一份修正，再次儲存會取代上一份；重新演練會清除，可先匯出城市存檔留存。</p></details></section>`:'';
  ...
  const body=!v.decision?`
   ...
   <div class="adventure-roles">${v.roles.map((r,i)=>`<div class="adventure-role ${r.visited?'is-visited':''}"><button ${action(id,'visit',`data-adventure-role="${esc(r.id)}"`)} ${r.visited?'disabled':''}><span aria-hidden="true">${r.visited?'✓':i+1}</span> ${esc(residents[i]?`${residents[i].name}・${residents[i].role}`:r.name)}${r.visited?'・已訪查':'・聽線索'}${r.visited?'':'<small>耗時 1</small>'}</button>${r.visited?`<p>${esc(r.clue)}</p>`:''}</div>`).join('')}</div>
  ...
  return `<section class="adventure-panel" data-adventure-panel="${esc(id)}" aria-labelledby="adventure-title-${esc(id)}"><div class="adventure-heading"><p class="adventure-kicker">汴水文學冒險 · 第 ${esc(v.round+1)} 次演練</p><h4 id="adventure-title-${esc(id)}">${esc(v.title)}</h4><p class="adventure-mission">${esc(v.mission)}</p></div><div class="adventure-resources" aria-label="本次演練資源"><span>可用時間 <strong>${esc(v.resources.time)}</strong></span><span>援助物資 <strong>${esc(v.resources.aid)}</strong></span><small>本篇演練專用</small></div>${continuityPanel}${body}<p class="adventure-status" role="status" tabindex="-1">${esc(messages.get(id)||'')}</p><details class="adventure-history"><summary>我的冒險歷程與收藏（${history.length} 次決策）</summary>${collection.length?`<p>已收藏：${collection.map(esc).join('、')}</p>`:'<p>收藏櫃等待你的第一段故事。</p>'}${history.length?`<ol>${history.map(h=>`<li><strong>第 ${esc(h.round+1)} 次：</strong>${esc(v.choices.find(c=>c.id===h.choiceId)?.label||h.choiceId)}${h.reason?`<p>${esc(h.reason)}</p>`:''}<p>後續：${h.followup? h.followup.choiceId==='repair'?'投入剩餘資源回應需要':'投入另一群居民的保障':'尚未安排'}</p></li>`).join('')}</ol>`:'<p>做出選擇後，就會留下紀錄。</p>'}${v.decision?`<p>再試一次會補足本篇演練資源。保留最近12次決策與已得收藏，重玩超過12次會移除最早一筆。</p>${id==='yueyang'?'<p>重新演練會清除本輪修正與草稿；要留存已儲存的修正，請先匯出城市存檔。</p>':''}<button ${action(id,'reset')}>換個安排再演練</button>`:''}</details><p class="adventure-note">教學改編 · ${esc(v.note)}</p></section>`;
 }
 function input(event){
  const el=event.target,field=el?.dataset?.adventureField;if(!['reason','reflection','evidence'].includes(field))return;
  sync();
  if(field!=='reason'){
   const id=el.dataset.adventureId,v=adventureView(getTown(),id),c=continuityView(getTown(),id);if(!v||!c?.branchId)return;
   const key=roundKey(id,v.round),draft={...(reflections.get(key)||c.record||{evidenceId:'',reason:''})};
   if(field==='reflection')draft.reason=el.value.slice(0,600);else draft.evidenceId=el.value;
   reflections.set(key,draft);
   const status=el.closest('[data-adventure-panel]')?.querySelector('[data-adventure-reflection-draft]');
   if(status)status.textContent='修正草稿保留中；切換篇章仍保留，重新整理前請按下儲存。';
   return;
  }
  drafts.set(el.dataset.adventureId,el.value.slice(0,400));
 ...
 function refresh(panel,id,message,name){
  ...
  const continuityOpen=panel.querySelector('[data-adventure-continuity]')?.open;
  ...
  if(continuityOpen&&replacement.querySelector('[data-adventure-continuity]'))replacement.querySelector('[data-adventure-continuity]').open=true;
  const focusTarget=['reflect','failed'].includes(name)?replacement.querySelector('.adventure-status'):name==='decide'?replacement.querySelector('.adventure-outcome h5'):name==='resolve'?replacement.querySelector('.adventure-followup h5'):name==='claim'?replacement.querySelector('.adventure-reward h5'):replacement.querySelector('[data-adventure-action=visit]:not(:disabled),[data-adventure-action=decide]');
 ...
 function click(event){
  ...
  sync();
  const key=roundKey(id,adventureView(getTown(),id)?.round);
  const operations={reflect:t=>recordContinuity(t,id,reflections.get(key)||continuityView(t,id)?.record||{}),visit:t=>visitAdventure(t,id,b.dataset.adventureRole),decide:t=>decideAdventure(t,id,{choiceId:b.dataset.adventureChoice,reason:drafts.get(id)||''}),resolve:t=>resolveAdventure(t,id,{choiceId:b.dataset.adventureChoice}),reset:t=>resetAdventure(t,id),claim:t=>claimAdventure(t,id)};
  if(!operations[name])return false;
  const ok=change(operations[name]);
  if(ok&&(name==='decide'||name==='reset'))drafts.delete(id);
  if(ok&&['reflect','reset'].includes(name))reflections.delete(key);
  const success={reflect:'修正與原文證據已儲存；理解留待討論。',visit:'居民的線索已記錄。',decide:'選擇已儲存，看看居民的回應。理由保留待討論。',resolve:'後續安排已儲存；看看居民的回應與剩餘資源。',reset:'新演練已開始。保留最近12次決策與已得收藏，重玩超過12次會移除最早一筆。',claim:'本篇收藏已儲存。'};
  const failure=name==='reflect'?'這次修正尚未儲存。請選擇原文，填寫 1–600 字理由，並查看存檔通知；草稿仍保留。':'這次操作尚未儲存。請查看資源、前置條件與存檔通知；草稿仍保留。';
  refresh(panel,id,ok?success[name]:failure,ok?name:'failed');return true;
 }
