...
export function shelterResident(t,p,dt){
 ...
 if(!p.shelter){
  const buildings=t.buildings.filter(b=>b.stage>=3);
  const b=!p.outside?t.building(p.current):buildings.sort((a,b)=>Math.hypot(a.entrance[0]-p.x,a.entrance[1]-p.z)-Math.hypot(b.entrance[0]-p.x,b.entrance[1]-p.z))[0];
